...
function filter(searchText) {
  ...
  recipFiltered = recipFiltered.filter((recette) => {
    let test = recette.ingredientFlattened
      .toLowerCase()
...
function filterAlternative(searchText) {
  let filter = new Set();
  ...
  filter = recipFiltered.filter((recette) => {
    return recette.ingredientFlattened
      .toLowerCase()
      .includes(searchText.toLowerCase());
  });

  filter = filter.filter((recip) => {
    return recip.name.toLowerCase().indexOf(searchText.toLowerCase()) !== -1;
  });
...
function searchMainBarAlternate(search) {
  ...
  const searchFilter = filter(search.value);
...
/*
function filterWithSearchContent(search, filterMethod) {
  //J'efface les anciennes recette
  const recipContent = document.querySelector(".recetteContent");
  recipContent.innerHTML = "";

  const searchFilter = filterMethod(search.value);
...
filterWithSearchContent("", filter)
filterWithSearchContent("", filterAlternative)
*/
